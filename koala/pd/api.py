...
def __format_result(
    code: int, body: bytes, result_type: Type[ResultType]
) -> ResultType:
    if len(body) == 0:
        body = b"{}"
    if code == 200:
...
def set_pd_address(address: str):
    ...
    PD_ID_NEW_SERVER_URL = PD_ID_NEW_SERVER_URL.replace(old_address, __PD_ADDRESS)
    PD_ID_NEW_SEQUENCE_URL = PD_ID_NEW_SEQUENCE_URL.replace(old_address, __PD_ADDRESS)
...
async def keep_alive(
    server_id: int, lease_id: int, load: int
...
async def find_actor_position(
    actor_type: str, actor_id: str, ttl: int
...
if __name__ == "__main__":
    ...
    async def main():
        ...
        register_result = await register_server(
            server_id.id,
            int(time.time()),
            ttl=45,
            address="127.0.0.1:9999",
            services={"ITest": "TestImpl"},
            desc="debug server",
            load=1,
        )
